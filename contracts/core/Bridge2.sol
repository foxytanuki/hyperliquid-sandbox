...
contract Bridge2 is Pausable, ReentrancyGuard {
    ...
    function requestWithdrawal(
        ...
        bytes32 data = keccak256(
            abi.encode("requestWithdrawal", user, destination, usd, nonce)
        );
    ...
    function makeMessage(bytes32 data) private view returns (bytes32) {
        Agent memory agent = Agent(
            "a",
            keccak256(abi.encode(address(this), data))
        );
        return hash(agent);
    }
    ...
    function modifyLocker(
        ...
        bytes32 data = keccak256(
            abi.encode("modifyLocker", locker, _isLocker, nonce)
        );
        bytes32 message = makeMessage(data);
    ...
    function modifyFinalizer(
        ...
        bytes32 data = keccak256(
            abi.encode("modifyFinalizer", finalizer, _isFinalizer, nonce)
        );
        bytes32 message = makeMessage(data);
    ...
    function invalidateWithdrawals(
        ...
        bytes32 data = keccak256(
            abi.encode("invalidateWithdrawals", messages, nonce)
        );
        bytes32 message = makeMessage(data);
